...
public class ProductDetailsWindow extends Stage {
	...
	public ProductDetailsWindow(ProductListWindow list, Product selectedProduct) {
		...
		List<String> displayValues = Arrays.asList(selectedProduct.getProductName(),
				...
				(new Integer(selectedProduct.getQuantityAvail())).toString(), selectedProduct.getDescription());
		FourByTwoGridPane dataTable = new FourByTwoGridPane(DefaultData.DISPLAY_PRODUCT_FIELDS, displayValues,
				"darkred", GuiConstants.PROD_DETAILS_GRID_WIDTH);
		...
		addToCartButton.setOnAction(evt -> {
			...
			ObservableList<ShoppingCart> cartData = FXCollections.observableArrayList(cart);
